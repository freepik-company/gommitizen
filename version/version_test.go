...
func TestVersionData_GetGit(t *testing.T) {
	...
	mockGit.EXPECT().GetDirPath().Return("/tmp").AnyTimes()
	...
	assert.Equal(t, mockGit.GetDirPath(), version.GetGit().GetDirPath(), "The git directory should be the default one")
}
